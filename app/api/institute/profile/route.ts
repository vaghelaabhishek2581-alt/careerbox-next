...
export async function GET (req: NextRequest) {
  ...
  try {
    ...
    const subscriptionInfo = subscription
      ? {
          ...
          features: {
            ...
            analyticsAccess: subscription.features?.analyticsAccess || false,
    ...
    // Return institute with subscription info
    return NextResponse.json(
      {
        ...institute.toObject(),
...
export async function PATCH (req: NextRequest) {
  try {
    ...
    const instituteId = new Types.ObjectId(
      user.ownedOrganizations[0].toString()
    )
    ...
    if (
      ...
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(updateData.contact.email)
    ) {
      return NextResponse.json(
    ...
    if (researchAndInnovation)
      addToUpdates('researchAndInnovation', researchAndInnovation)
    if (alumniNetwork) addToUpdates('alumniNetwork', alumniNetwork)
    if (faculty_student_ratio)
      addToUpdates('faculty_student_ratio', faculty_student_ratio)
