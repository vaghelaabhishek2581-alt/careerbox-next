...
const highlightSchema = z.object({
  ...
  example: z
    .string()
    .max(200, "Example must be less than 200 characters")
    .optional(),
  ...
  try {
    ...
    if (!isAdmin && !isOwner) {
      return NextResponse.json(
        {
          error:
            "Access denied. Only admins and institute owners can access this resource.",
        },
        { status: 403 }
      );
    }
  ...
  try {
    ...
    if (!isAdmin && !isOwner) {
      return NextResponse.json(
        {
          error:
            "Access denied. Only admins and institute owners can modify this resource.",
        },
        { status: 403 }
      );
    }
...
export async function PATCH(
  ...
  try {
    ...
    if (!updatedInstitute) {
      return NextResponse.json(
        { error: "Institute or highlight not found" },
        { status: 404 }
      );
    }

    const updatedHighlight = updatedInstitute.highlights.find(
