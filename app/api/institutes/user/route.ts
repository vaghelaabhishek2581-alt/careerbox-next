...
export async function GET (req: NextRequest) {
  try {
    ...
    const institutesRaw = await AdminInstitute.find({ userIds: userId })
      .select('-__v')
      .lean()
      .exec()
    ...
    const subscriptions = await Subscription.find({
      organizationId: { $in: instituteIds },
    ...
    const transformedInstitutes = institutes.map(institute => {
      ...
      return {
        ...
        isVerified:
          institute.status === 'active' || institute.status === 'Active',
      ...
      data: {
        ...
        subscription: subscription
          ? {
              ...
              planType: subDoc.planType,
              planName: subDoc.planName,
              features: subDoc.features,
              endDate: subDoc.endDate,
              grantReason: subDoc.grantReason
            }
          : null,
        subscriptionId: subscription ? String(subDoc._id) : null
      },
