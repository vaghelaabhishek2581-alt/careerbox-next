...
async function getPublicProfileHandler(
  ...
  try {
    ...
    if (!userProfile) {
      return createErrorResponse("Profile not found", 404, "PROFILE_NOT_FOUND");
    }

    // Check if profile is public
    const isPublic =
      userProfile.profile?.preferences?.privacy?.profileVisible !== false;

    if (!isPublic) {
      return createErrorResponse("Profile is private", 403, "PROFILE_PRIVATE");
    }
    ...
    const publicProfile = {
      ...
      personalDetails: {
        firstName:
          userProfile.profile?.personalDetails?.firstName ||
          userProfile.name?.split(" ")[0] ||
          "",
        lastName:
          userProfile.profile?.personalDetails?.lastName ||
          userProfile.name?.split(" ").slice(1).join(" ") ||
          "",
        professionalHeadline:
          userProfile.profile?.personalDetails?.professionalHeadline ||
          "Professional",
        publicProfileId:
          userProfile.profile?.personalDetails?.publicProfileId || "",
      ...
      workExperiences: (userProfile.profile?.workExperience || []).map(
        (work: any) => ({
...
async function updateProfileHandler(
  ...
  try {
    ...
    const profileOwner = await db
      .collection("users")
      .findOne({ "profile.personalDetails.publicProfileId": profileId });
    ...
    const result = await db
      .collection("users")
      .updateOne({ _id: new ObjectId(user._id) }, { $set: updateData });
