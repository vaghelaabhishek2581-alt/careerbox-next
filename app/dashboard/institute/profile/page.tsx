...
import { fetchUserInstitutes, uploadInstituteImage } from "@/lib/redux/slices/instituteSlice";
import { Building2, MapPin, Phone, Mail, Globe, Users, BookOpen, Award, Camera, Save, Edit, Upload, X, Loader2 } from "lucide-react";
...
export default function InstituteProfile() {
  ...
  // Redux state
  const { selectedInstitute, isUploadingImage, loading, error } = useSelector((state: RootState) => state.institute);
  ...
  useEffect(() => {
    if (selectedInstitute) {
      setInstituteData({
        ...
        address: `${selectedInstitute.address?.street || ""}, ${selectedInstitute.address?.city || ""}, ${selectedInstitute.address?.state || ""}, ${selectedInstitute.address?.country || ""} ${selectedInstitute.address?.zipCode || ""}`.trim(),
        phone: selectedInstitute.phone || "",
        email: selectedInstitute.email || "",
        website: selectedInstitute.website || "",
        establishmentYear: selectedInstitute.establishmentYear?.toString() || "",
  ...
  const handleUploadLogoImage = async () => {
    if (selectedLogoFile && selectedInstitute) {
      try {
        await dispatch(uploadInstituteImage({ 
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <h3 className="mt-2 text-sm font-semibold text-gray-900">No institute selected</h3>
        ...
        <Card>
          <CardContent className="p-0">
            <div 
              ...
              style={{
                backgroundImage: selectedInstitute?.coverImage ? `url(${selectedInstitute.coverImage})` : undefined,
              }}
            >
            ...
            <div className="relative px-6 pb-6">
              <div className="flex items-end gap-6 -mt-16">
                <div className="relative">
                  <Avatar className="w-32 h-32 border-4 border-white shadow-lg">
                    ...
                    <AvatarFallback className="bg-orange-600 text-white text-2xl font-bold">
                      {selectedInstitute?.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                    </AvatarFallback>
                  </Avatar>
                  <Button 
                    size="sm" 
                ...
                <div className="flex-1 mt-4">
                  ...
                  <p className="text-gray-600 mb-3">{selectedInstitute?.description}</p>
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    <span className="flex items-center gap-1">
                      <MapPin className="w-4 h-4" />
                      {selectedInstitute?.address?.city}, {selectedInstitute?.address?.state}
                    </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <p className="text-2xl font-bold text-gray-900">{selectedInstitute?.studentCount || 0}</p>
          ...
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <p className="text-2xl font-bold text-gray-900">{selectedInstitute?.courseCount || 0}</p>
          ...
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <p className="text-2xl font-bold text-gray-900">{selectedInstitute?.facultyCount || 0}</p>
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Preview Institute Logo</h3>
                <Button
                  variant="ghost"
                  size="sm"
