...
function PlacementsSection({ placementData, topRecruiters, sectors }: any) {
  if (!placementData && (!topRecruiters || topRecruiters.length === 0) && (!sectors || sectors.length === 0)) {
    return null
  }
      ...
      <CardContent>
        {placementData && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            {placementData.averageSalary && placementData.averageSalary !== '₹' && placementData.averageSalary !== 'N/A' && (
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-2">
                  <TrendingUp className="h-6 w-6 text-blue-600" />
                  <div className="text-sm text-blue-600 font-medium">Average Package</div>
                </div>
                <div className="text-2xl font-bold text-blue-700">{placementData.averageSalary}</div>
              </div>
            )}
            {placementData.highestSalary && placementData.highestSalary !== '₹' && placementData.highestSalary !== 'N/A' && (
              <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-2">
                  <Award className="h-6 w-6 text-green-600" />
                  <div className="text-sm text-green-600 font-medium">Highest Package</div>
                </div>
                <div className="text-2xl font-bold text-green-700">{placementData.highestSalary}</div>
              </div>
            )}
            {placementData.companiesVisited && placementData.companiesVisited !== '₹' && placementData.companiesVisited !== 'N/A' && placementData.companiesVisited !== 0 && (
              <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-2">
                  <Building2 className="h-6 w-6 text-purple-600" />
                  <div className="text-sm text-purple-600 font-medium">Companies Visited</div>
                </div>
                <div className="text-2xl font-bold text-purple-700">{placementData.companiesVisited}</div>
              </div>
            )}
            {placementData.overallPlacementRate && placementData.overallPlacementRate !== '₹' && placementData.overallPlacementRate !== 'N/A' && placementData.overallPlacementRate !== 0 && (
              <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-2">
                  <Users className="h-6 w-6 text-orange-600" />
                  <div className="text-sm text-orange-600 font-medium">Placement Rate</div>
                </div>
                <div className="text-2xl font-bold text-orange-700">{placementData.overallPlacementRate}%</div>
              </div>
            )}
          </div>
        )}
        ...
        {topRecruiters && topRecruiters.length > 0 && (
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {topRecruiters.map((recruiter: string, index: number) => (
      ...
      <CardContent className="space-y-4">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-700 mb-4">
            Start your journey with us. Fill out the application form to get started.
          </p>
          <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="border rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600">{contact.phone.join(', ')}</p>
          </div>
          ...
          <div className="border rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600">{contact.admissionsEmail || contact.email}</p>
          </div>
        </div>
      </CardContent>
    </Card>
...
export function InstituteDetailPage({ institute }: InstituteDetailPageProps) {
  ...
  const allCourses = useMemo(() => {
    ...
    return institute.programmes.flatMap(programme =>
  ...
  const filterOptions = useMemo(() => {
    if (!selectedProgramme?.course) return { educationTypes: [], courseLevels: [], durations: [], exams: [] }
  ...
  const filteredCourses = useMemo(() => {
    ...
    return selectedProgramme.course.filter((course: any) => {
      ...
      if (selectedEducationType !== 'all' && course.educationType !== selectedEducationType) {
        return false
      }

      // Course level filter
      if (selectedCourseLevel !== 'all' && course.level !== selectedCourseLevel) {
        return false
      }
  ...
  }, [selectedProgramme, courseSearchQuery, selectedEducationType, selectedCourseLevel, selectedDuration, selectedFeeRange, selectedExam])
  ...
  const getLatestPlacementData = () => {
    if (institute.placements && ('averageSalary' in institute.placements || 'highestSalary' in institute.placements || 'overallPlacementRate' in institute.placements)) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-28">
      ...
      <div className="relative">
        ...
        {placementData && (
          placementData.averageSalary || placementData.highestSalary || placementData.overallPlacementRate || placementData.companiesVisited
        ) && (
            <div className="container mx-auto px-4 -mt-8 lg:-mt-16 relative z-20 mb-4 lg:mb-8">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-6">
                ...
                {placementData.overallPlacementRate && placementData.overallPlacementRate !== '₹' && placementData.overallPlacementRate !== 'N/A' && (
                ...
                {placementData.companiesVisited && placementData.companiesVisited !== '₹' && placementData.companiesVisited !== 'N/A' && (
        ...
        <div className="space-y-6">
          ...
          <div className="hidden lg:grid lg:grid-cols-4 gap-6 lg:gap-8">
            <div className="lg:col-span-3 space-y-6">
              <Card>
                ...
                <CardContent>
                  {(() => {
                    ...
                    if ((institute as any).overview?.description) {
                      return (
                        <p className="text-gray-700 leading-relaxed">
                            ...
                            <div key={index} className="bg-gradient-to-br from-blue-50 via-indigo-50 to-blue-100 p-5 rounded-xl border border-blue-200 shadow-sm hover:shadow-md transition-all hover:scale-[1.02] cursor-pointer"
                              ...
                              <div className="flex items-start justify-between mb-3">
                                <div className="flex-1">
                                  <h3 className="text-lg font-bold text-blue-900 mb-1">{programme.name}</h3>
                                  {programme.courseCount > 0 && (
                                    <div className="text-sm text-blue-600 font-medium">
                                      {programme.courseCount} Course{programme.courseCount !== 1 ? 's' : ''}
                                    </div>
                                  )}
                                </div>
                                {programme.placementRating && programme.placementRating > 0 && (
                                  <div className="flex items-center gap-1 bg-green-100 px-2 py-1 rounded-full">
                                    ...
                                    <span className="text-xs font-bold text-green-700">{programme.placementRating}/5</span>
                                  </div>
                                )}
                              </div>
                              ...
                              {programme.eligibilityExams && programme.eligibilityExams.length > 0 && (
                                <div className="mb-3">
                                  <div className="text-xs text-gray-600 mb-2 font-medium">Eligibility Exams:</div>
                                  <div className="flex flex-wrap gap-1">
                                    {programme.eligibilityExams.slice(0, 3).map((exam: string, examIndex: number) => (
                                      <span key={examIndex} className="text-xs bg-white px-2 py-1 rounded-full border border-blue-200 text-blue-700 font-medium">
                                        {exam}
                                      </span>
                                    ))}
                                    {programme.eligibilityExams.length > 3 && (
                              ...
                              {programme.course && programme.course.length > 0 && (
                                <div className="text-xs text-gray-600">
                                  <div className="font-medium mb-1">Sample Courses:</div>
                                  <div className="space-y-1">
                                    {programme.course.slice(0, 2).map((course: any, courseIndex: number) => (
                                      <div key={courseIndex} className="flex justify-between items-center">
                                        <span className="truncate">{course.name || course.degree}</span>
                                        {course.duration && (
                                    ...
                                    {programme.course.length > 2 && (
                                      <div className="text-gray-500 italic">+{programme.course.length - 2} more courses</div>
                                    )}
                                  </div>
                                </div>
                              )}
                    ...
                    <Card>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-600 mt-1">
                                {filteredCourses.length} of {selectedProgramme.course?.length || 0} courses
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      <CardContent>
                        ...
                        <div className="mb-6 space-y-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                            ...
                            <Select value={selectedFeeRange} onValueChange={setSelectedFeeRange}>
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="100k-300k">₹1L - ₹3L</SelectItem>
                                <SelectItem value="300k-500k">₹3L - ₹5L</SelectItem>
                          ...
                          {(selectedEducationType !== 'all' || selectedCourseLevel !== 'all' || selectedDuration !== 'all' || selectedFeeRange !== 'all' || selectedExam !== 'all') && (
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-sm text-gray-600">Active filters:</span>
                              {selectedEducationType !== 'all' && (
                                <Badge variant="secondary" className="gap-1">
                                  {selectedEducationType}
                                  <X className="h-3 w-3 cursor-pointer" onClick={() => setSelectedEducationType('all')} />
                                </Badge>
                              )}
                              {selectedCourseLevel !== 'all' && (
                                <Badge variant="secondary" className="gap-1">
                                  {selectedCourseLevel}
                                  <X className="h-3 w-3 cursor-pointer" onClick={() => setSelectedCourseLevel('all')} />
                                </Badge>
                              )}
                              ...
                              {selectedFeeRange !== 'all' && (
                                <Badge variant="secondary" className="gap-1">
                                  {selectedFeeRange === 'under-100k' ? 'Under ₹1L' :
                                    selectedFeeRange === '100k-300k' ? '₹1L-₹3L' :
                                      selectedFeeRange === '300k-500k' ? '₹3L-₹5L' :
                                        selectedFeeRange === '500k-1m' ? '₹5L-₹10L' : 'Above ₹10L'}
                                  <X className="h-3 w-3 cursor-pointer" onClick={() => setSelectedFeeRange('all')} />
                                </Badge>
                              )}
                              {selectedExam !== 'all' && (
                                <Badge variant="secondary" className="gap-1">
                        ...
                        {filteredCourses.length > 0 ? (
                          <div className="space-y-4">
                            {filteredCourses.map((course: any, index: number) => (
                              <div key={index} className="border rounded-xl p-6 hover:shadow-lg transition-all bg-white">
                                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                  {/* Course Info */}
                                  <div className="flex-1">
                                    <div className="flex items-start justify-between mb-3">
                                      <div className="flex-1">
                                        <h3 className="text-xl font-bold text-gray-900 mb-1">{course.degree}</h3>
                                        {course.name && course.name !== course.degree && (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                      ...
                                      {course.affiliatedUniversity && course.affiliatedUniversity !== institute.name && (
                                        <div className="flex items-center gap-2 text-sm text-gray-600">
                                          <Award className="h-4 w-4" />
                                          <span>{course.affiliatedUniversity}</span>
                                        </div>
                                      )}
                                    </div>

                                    {/* Eligibility Exams */}
                                    {course.eligibilityExams && course.eligibilityExams.length > 0 && (
                                      <div className="mb-4">
                                        <p className="text-xs text-gray-600 mb-2 font-medium">Eligibility Exams:</p>
                                        <div className="flex flex-wrap gap-2">
                                          {course.eligibilityExams.map((exam: string, examIndex: number) => (
                                            <Badge key={examIndex} variant="secondary" className="text-xs">
                                              {exam}
                                            </Badge>
                                          ))}
                                        </div>
                                      ...
                                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 p-4 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg">
                                        {course.placements.averagePackage && (
                                          <div>
                                            ...
                                            <p className="text-sm font-bold text-green-600">₹{(course.placements.averagePackage / 100000).toFixed(2)}L</p>
                                          </div>
                                        )}
                                        {course.placements.highestPackage && (
                                          <div>
                                            <p className="text-xs text-gray-600">Highest Package</p>
                                            <p className="text-sm font-bold text-blue-600">₹{(course.placements.highestPackage / 100000).toFixed(2)}L</p>
                                          </div>
                                        )}
                                        {course.placements.placementRate && (
                                          <div>
                                            <p className="text-xs text-gray-600">Placement Rate</p>
                                            <p className="text-sm font-bold text-purple-600">{course.placements.placementRate}%</p>
                                          </div>
                                        )}
                                      </div>
                                    )}
                                  ...
                                  <div className="lg:w-64 flex flex-col gap-4">
                                    {course.fees && (course.fees.tuitionFee || course.fees.totalFee) && (
                                      <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-xl p-4 border border-orange-200">
                                        <div className="flex items-center gap-2 mb-2">
                                          <CreditCard className="h-4 w-4 text-orange-600" />
                                          <span className="text-xs text-orange-600 font-medium">Course Fees</span>
                                        </div>
                                        {course.fees.tuitionFee && (
                                          <div className="mb-1">
                                            <p className="text-xs text-gray-600">Tuition Fee</p>
                                            <p className="text-2xl font-bold text-orange-700">₹{(course.fees.tuitionFee / 100000).toFixed(2)}L</p>
                                          </div>
                                        )}
                                        {course.fees.totalFee && course.fees.totalFee !== course.fees.tuitionFee && (
                                          <div>
                                            <p className="text-xs text-gray-600">Total Fee</p>
                                            <p className="text-lg font-semibold text-orange-600">₹{(course.fees.totalFee / 100000).toFixed(2)}L</p>
                                          </div>
                                        )}
                                        {course.fees.currency && course.fees.currency !== 'INR' && (
                                          <p className="text-xs text-gray-500 mt-1">Currency: {course.fees.currency}</p>
                                        )}
                                      </div>
                                    )}
                                    ...
                                    <div className="flex flex-col gap-2">
                                      ...
                                      {course.seoUrl && (
                                        ...
                                        >
                                          <a href={course.seoUrl} target="_blank" rel="noopener noreferrer">
                                            More Details →
                                          </a>
                                        </Button>
                                      )}
              ...
              {institute.rawOverview && Array.isArray(institute.rawOverview) && institute.rawOverview.length > 0 && (
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {institute.rawOverview.map((item: any, index: number) => (
                        item && item.key && item.value && (
                          <div key={index} className="bg-gradient-to-br from-yellow-50 via-orange-50 to-amber-50 p-4 rounded-xl border border-yellow-200 shadow-sm hover:shadow-md transition-shadow">
                            <div className="text-sm text-gray-600 mb-1 font-medium">{item.key}</div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 text-sm lg:text-base">Established</span>
                    <span className="font-semibold text-sm lg:text-base">{institute.establishedYear}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 text-sm lg:text-base">Type</span>
                    <span className="font-semibold text-sm lg:text-base">{institute.type}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 text-sm lg:text-base">Location</span>
                    <span className="font-semibold text-sm lg:text-base">{institute.location.city}, {institute.location.state}</span>
                  </div>
                  {(institute as any).campusDetails?.totalArea && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600 text-sm lg:text-base">Campus Area</span>
                      <span className="font-semibold text-sm lg:text-base">{(institute as any).campusDetails.totalArea}</span>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-sm">{institute.establishedYear}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 text-sm">Type</span>
                  <span className="font-semibold text-sm">{institute.type}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 text-sm">Location</span>
                  <span className="font-semibold text-sm">{institute.location.city}, {institute.location.state}</span>
                </div>
                {(institute as any).campusDetails?.totalArea && (
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 text-sm">Campus Area</span>
                    <span className="font-semibold text-sm">{(institute as any).campusDetails.totalArea}</span>
                  </div>
                )}
              </CardContent>
            </Card>
          </div>
          {(institute as any).overview && Array.isArray((institute as any).overview) && (institute as any).overview.length > 0 && (
            <KeyValueTable
              title="Institute Overview"
              data={(institute as any).overview}
          ...
          {(institute as any).academics?.programOverviews && (institute as any).academics.programOverviews.length > 0 && (
            <KeyValueTable
              ...
              data={(institute as any).academics.programOverviews}
          ...
          {(institute as any).campusDetails && (
            <>
              {((institute as any).campusDetails.campusType || (institute as any).campusDetails.environment || (institute as any).campusDetails.totalArea) && (
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {(institute as any).campusDetails.campusType && (
                        <div className="flex items-center gap-3 p-3 rounded-lg border bg-card">
                          ...
                          <div>
                            <div className="text-sm text-muted-foreground">Campus Type</div>
                            <div className="font-medium">{(institute as any).campusDetails.campusType}</div>
                          </div>
                        </div>
                      )}
                      {(institute as any).campusDetails.environment && (
                        <div className="flex items-center gap-3 p-3 rounded-lg border bg-card">
                          ...
                          <div>
                            ...
                            <div className="font-medium">{(institute as any).campusDetails.environment}</div>
                          </div>
                        </div>
                      )}
                      {(institute as any).campusDetails.totalArea && (
                        <div className="flex items-center gap-3 p-3 rounded-lg border bg-card">
                          ...
                          <div>
                            ...
                            <div className="font-medium">{(institute as any).campusDetails.totalArea}</div>
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <FacilityList
                ...
                facilitiesArray={(institute as any).campusDetails?.facilities_arr}
              />
            </>
          )}
          ...
          <RankingsSection
            ...
            rankingsDescription={(institute as any).rankings?.rankingsDescription}
          ...
          {(institute as any).rankings?.data && (institute as any).rankings.data.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {(institute as any).rankings.data.map((ranking: any, index: number) => (
