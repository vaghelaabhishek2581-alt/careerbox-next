...
import type { ProgramResult } from '@/lib/actions/unified-recommendations'
        ...
        <CardContent className="p-5">
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {program.institute?.location && (
                <div className="flex items-center gap-1 mt-1 text-sm text-gray-500">
                  ...
                  <span>{program.institute.location.city}, {program.institute.location.state}</span>
                </div>
              )}
              ...
              {program.eligibilityExams && program.eligibilityExams.length > 0 && (
                <div className="mt-3">
                  <p className="text-xs font-semibold text-gray-700 mb-1">Eligibility Exams:</p>
                  <div className="flex flex-wrap gap-1">
                    {program.eligibilityExams.slice(0, 5).map((exam, index) => (
                      <Badge key={index} variant="outline" className="text-xs bg-blue-50 text-blue-700 border-blue-200">
                        {exam}
                      </Badge>
                    ))}
                    {program.eligibilityExams.length > 5 && (
                      <span className="text-xs text-gray-500">+{program.eligibilityExams.length - 5}</span>
                    )}
                  </div>
                </div>
              )}
              ...
              {program.courses && program.courses.length > 0 && (
                <div className="mt-3 space-y-1">
                  <p className="text-xs font-semibold text-gray-700">Sample Courses:</p>
                  {program.courses.slice(0, 2).map((course: any, index: number) => (
                    <div key={index} className="flex items-start justify-between gap-2 text-xs bg-gray-50 rounded p-2">
            ...
            <div className="flex flex-col justify-between">
              <Button size="sm" asChild className="bg-purple-600 hover:bg-purple-700 whitespace-nowrap">
                <Link href={`/recommendation-collections/${program.institute?.slug || program.id}`}>
                  View Details
                </Link>
              </Button>
            </div>
  ...
  return (
    <Card className="hover:shadow-xl transition-all duration-300 border-0 shadow-md group">
      ...
      <CardContent className="pt-0 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {program.courseCount > 0 && (
              <div className="bg-purple-50 rounded-lg p-3">
                ...
                <div className="text-2xl font-bold text-purple-700">{program.courseCount}</div>
              </div>
            )}
            ...
            {program.placementRating && program.placementRating > 0 && (
              <div className="bg-yellow-50 rounded-lg p-3">
                ...
                <div className="text-2xl font-bold text-yellow-700">{program.placementRating}/5</div>
              </div>
            )}
          </div>
        )}
        ...
        {program.eligibilityExams && program.eligibilityExams.length > 0 && (
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-2">Eligibility Exams</h4>
        ...
        {program.courses && program.courses.length > 0 && (
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-2">Sample Courses</h4>
            <div className="space-y-2">
              {program.courses.slice(0, 3).map((course) => (
                <div key={course.id} className="flex items-center justify-between text-sm bg-gray-50 rounded p-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <GraduationCap className="h-4 w-4 text-gray-400 flex-shrink-0" />
                    <span className="font-medium text-gray-700 truncate">{course.degree || course.name}</span>
                  </div>
                  {course.fees?.totalFee && course.fees.totalFee > 0 && (
