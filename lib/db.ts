...
const MONGODB_URI = 'mongodb+srv://jon:vatsal00@cluster0.esqiu8v.mongodb.net/career-box?retryWrites=true&w=majority';
const MONGODB_DB = 'career-box-001';
...
export async function createCounsellingRequest(requestData: {
  ...
  const result = await db.collection('counselling_requests').insertOne(requestWithDefaults);
  return result;
}
...
export async function updateCounsellingRequest(id: string, updateData: any) {
  const { db } = await connectToDatabase();
  return await db.collection('counselling_requests').updateOne(
    { _id: id },
...
export async function createOrganization(organizationData: {
  ...
  const result = await db.collection('organizations').insertOne(organizationWithDefaults);
  return result;
}
...
export async function createBusiness(businessData: {
  ...
  const result = await db.collection('businesses').insertOne(businessWithDefaults);
  return result;
}

export async function getBusinesses(page: number = 1, limit: number = 10, filters: any = {}) {
  const { db } = await connectToDatabase();
  const skip = (page - 1) * limit;
...
export async function getBusinessStats() {
  ...
  const totalRevenue = businesses.reduce((sum, business) => {
    ...
    if (business.subscription.status === 'active') {
      return sum + (planPricing[business.subscription.plan as keyof typeof planPricing] || 0);
    }
    return sum;
  }, 0);
