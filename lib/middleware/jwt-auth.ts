...
export function requireRole(user: any, allowedRoles: string[]): boolean {
  ...
  return (
    allowedRoles.includes(user.role) || allowedRoles.includes(user.activeRole)
  );
}
...
export function withJWTAuth(
  ...
  return async (request: NextRequest): Promise<NextResponse> => {
    try {
      ...
      if (
        options.requireBusinessOrInstitute &&
        !requireBusinessOrInstitute(user)
      ) {
        return NextResponse.json(
          {
            success: false,
...
// JWT Auth wrapper for dynamic routes (with params) - Next.js 15 compatible
export function withJWTAuthDynamic<T extends Record<string, string>>(
  ...
  return async (
    ...
    try {
      ...
      // Call the handler with authenticated user and async params
